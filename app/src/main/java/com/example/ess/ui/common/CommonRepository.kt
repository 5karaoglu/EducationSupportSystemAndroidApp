...
@Inject constructor(
    ...
    override fun downloadFile(submit: Submit) = callbackFlow<DataState<String>> {
        ...
        awaitClose {
            ref.removeOnProgressListener(progressListener).removeOnSuccessListener(successListener)
    ...
    override fun getMessages(chatID: String) = callbackFlow<DataState<List<Message>>> {
        ...
        awaitClose {
            firebaseDatabase.getReference("contacts/$chatID").removeEventListener(listener)
    ...
    suspend fun getContact(receiverUid: String) = withContext(Dispatchers.IO) {
        ...
        if (contact == null) {
            ...
            cUser = ss2.getValue(UserProfile::class.java)
    ...
    override suspend fun getActivities(user: UserProfile): Flow<DataState<List<ActivityItem>>> = flow {
        ...
        try {
            ...
            snapshot.children.forEach {
                it.getValue(ActivityItem::class.java)?.let { it1 ->
                    list.add(it1)
